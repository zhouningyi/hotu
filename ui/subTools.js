...
define(['./../utils/utils','zepto', './../render/renderer','anim','./lightSatSelector','./hueSlider','./slider'], function(Utils, $, Renderer,keyAnim, LightSatSelector, HueSlider, Slider) {
  ...
  function SubTools(opt) {
    ...
    this.status = 'select';
    this.subToolsNode.addClass('tools-out-left');
  }
  ...
  SubTools.prototype.init = function(brushes) {
    ...
    subToolsW = this.container.width() - this.bindNode.width() - 10,
    subToolsNode = this.subToolsNode = $(
      '<div class="sub-tools">\
    ...
    var colorNode = this.colorNode = subToolsNode.find('.color-ui')
    var shapeNode = this.shapeNode = subToolsNode.find('.shape-ui');
    var curBrush = this.curBrush;
    var controls = curBrush.controls;
    this.renderControl(controls);
  };

  SubTools.prototype.renderControl = function(controls){
    ...
    var obj, uiName, uiDesc, Constructor, containerName, constructorUI, container;
    for(var k in controls){
      obj = controls[k];
      constructorUI = obj.constructorUI;
      uiDesc = obj.uiDesc;
      Constructor = mapUI[constructorUI];
      containerName = obj.containerName;
      container = mapContainer[containerName];

      if(Constructor){
        new Constructor(container,uiDesc);
      }
    }
  };
  ...
  SubTools.prototype.in = function(obj ,cb) { //隐藏
    if (this.uiStatus!=='in'&&this.uiStatus!=='lock'&&obj) {
      ...
      subToolsNode.keyAnim('toolsInLeft', {
        'time': 0.4,
        'cb': function() {
          subToolsNode
          .css({
